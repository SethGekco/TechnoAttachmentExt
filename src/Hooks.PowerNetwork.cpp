...
namespace TAExtPowerNetwork
{
	...
	// Moved to AttachmentParsers.h so the render-side visibility check can share
	// it instead of keeping a second copy that could drift.
	static bool HouseRelationMatches(HouseClass* pSelf, HouseClass* pOther, int mask)
	{
		return TAExt_HouseRelationMatches(pSelf, pOther, mask);
	}
